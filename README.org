...
One line, after putting it on your =load-path=:
...
#+begin_src emacs-lisp
  (use-package garamond
    :load-path "~/src/garamond"
    :config (garamond-follow-declarations-mode 1))
#+end_src

That mode is the only global thing in the package, and it is not a spacing
-- it is a willingness to read the ones documents state.  It runs from
=hack-local-variables-hook=, so it sees a file just after the file's own
variables have been applied.  A file that declares nothing is left alone, and
so is a buffer nobody types in -- one that visits no file and cannot be
edited, such as Dired under a =.dir-locals.el= entry for all modes.  Nothing runs itself on load; a library that
switches itself on is a library you cannot load in order to read it.
